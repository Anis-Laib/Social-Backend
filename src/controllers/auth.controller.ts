...
export const login = async (req: Request, res: Response) => {
  ...
  const verifyPassword = await compare(value.password, user.passwordHash);

  if (!verifyPassword) throw new BadRequestError("Invalid email or password!");
...
export const refresh = async (req: Request, res: Response) => {
  try {
    ...
    if (!dbToken) throw new UnauthorizedError();
